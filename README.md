...
2. Add dependency `dev.reformator.stacktracedecoroutinator:stacktrace-decoroutinator-jvm:2.4.7` and call method `DecoroutinatorJvmApi.install()`.
3. Add `-javaagent:stacktrace-decoroutinator-jvm-agent-2.4.7.jar` to your JVM start arguments. Corresponding dependency is `dev.reformator.stacktracedecoroutinator:stacktrace-decoroutinator-jvm-agent:2.4.7`.
...
```kotlin
plugins {
    id("dev.reformator.stacktracedecoroutinator") version "2.4.7"
}
```
